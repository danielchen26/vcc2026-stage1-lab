...
| 可检出性（MDE）刻画 | ✅ 已实测 | 跨基因动态范围 **10.5×**（p95/p25）· [docs/02](docs/02-findings.md#f6) |
...
6. **答案的一半在输入里** —— $R_p = (\text{生物学}) \cap (\text{可检出性})$，
   而可检出性完全由发给你的对照细胞决定，跨基因动态范围 **10.5×**（p95/p25）。
   ⚠️ 这是操作门槛的**下界** —— 真实打分器还要在 9,929 个基因上做 BH。

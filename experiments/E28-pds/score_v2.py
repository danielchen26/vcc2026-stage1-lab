"""对 pred_v2 跑官方六指标，并与 E27 的 variant A / 退化基线对比。"""
...
def main():
    ...
    if not pq.exists():
        ...
        df = compute_metrics(str(OUT / "pred_v2.h5ad"), str(E27 / "real.h5ad"), config=cfg)
        aggregate_metrics(df).write_parquet(pq)
        print(f"compute_metrics 完成 {time.time()-t0:.0f}s")
    else:
        print("已有 agg_v2.parquet，跳过计算")

    res = score_metrics(str(wide(pq, OUT / "agg_v2_wide.csv")),
    ...
    print(f"\n{'指标':42s} {'V2 原始':>10s} {'A from_b':>9s} {'V2 from_b':>10s} {'变化':>9s}")
    print("-" * 88)
    for k in SCORED:
    ...
    print(f"\n领先者 0.1899   V2 {'超过 ✅' if fb['avg_score'] > 0.1899 else '未达'}"

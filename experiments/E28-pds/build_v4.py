"""变体 V4：真正的单旋钮实验 —— 只把 hamilton 换成 stochastic_round。

由 E27 的 build 逐字复制，仅两处改动：输出路径、design_cells 多传 quantizer="stochastic"。
gate、K 规则（含 n_src 在 gate∩common 上的 BH）、bootstrap、两点 psi_bar 构造、
20k 深度、种子全部不变。

为什么必须这样做：V2 一次改了三件事（量化器 + top1000 + 去面板均值），净损 -0.019；
V3 以为是单旋钮，实际还改了排序统计量（|beta| vs |beta|/SE）、K（重算的 n_src 不同）
和生成模型（丢掉 bootstrap 改纯多项式），净损 -0.123。两次都无法归因。

stochastic_round 只随机化小数部分，bootstrap 继承的过度离散/dropout 与两点构造
全部保留 —— 这是 hamilton 确定性死区（2264 个基因恒为 0）的最小修复。
...
LAMBDA = 1.0               # lfc 收缩系数；lfc_nmae 的旋钮
...
def build() -> None:
    ...
    for j, p in enumerate(pick):
        ...
        K = 29 if n_src < 5 else (288 if n_src < 100 else ref.G)   # E24 的 P3 先验分区
    ...
    for tag, blocks in (("pred_v4", X_pred),):

"""变体 V8：真正的单旋钮 —— lfc 收缩系数 0.5 → 0.7，其余逐字照 V7。

由 E27 run.py 逐字复制，仅三处：输出路径、排序统计量、只写 pred_v8。
...
OUT = Path(__file__).resolve().parents[1] / "E34-pds-decouple" / "out"
N_PERT = 8                 # 官方 compute_metrics ~100 s/扰动
ALPHA, Z_BH, SEED = 0.05, 3.184, 0
LAMBDA = 0.7               # V8 唯一改动：lfc 收缩系数 0.5 -> 0.7
# 假设：direction_reach 的 k* 受符号纯度上限约束，对 lambda **饱和**而非线性。
#   若饱和：lam 0.7 能同时保住 reach（lam=1.0 时 0.1424）与 expr_mse（lam=0.5 时 0.5988）
#   若线性：V7 已是最优，本变体会净损约 -0.002
# lambda_probe.py 已证明 pds 在 lam 0.7 与 1.0 完全相同（0.7143），故 pds 不承担代价。
...
def build() -> None:
    ...
    ntc_path.unlink(missing_ok=True)      # 磁盘只剩 ~2 GB，不能同时放 _ctrl 与 pred_v13
    ...
    print(f"集外通道 K_OFF={K_OFF} LAMBDA_OFF={LAMBDA_OFF} Bd 非零 {np.count_nonzero(Bd_panel):,}")
    ...
    for tag, blocks in (("pred_v13", X_pred),):
